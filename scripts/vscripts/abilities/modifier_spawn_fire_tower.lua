...
if modifier_spawn_fire_tower == nil then
	modifier_spawn_fire_tower = class({})
end

function modifier_spawn_fire_tower:IsHidden()
	return true
end

function modifier_spawn_fire_tower:OnCreated( kv )

	self.effect_aoe = self:GetAbility():GetSpecialValueFor( "spawn_effect_aoe" )
	self.spawn_delay_time = self:GetAbility():GetSpecialValueFor( "spawn_delay" )
...
function modifier_spawn_fire_tower:OnIntervalThink()
	if IsServer() then
